    ...
    <div className="admin-dark" style={{
      ...
      <div style={{ width: "100%", maxWidth: 420 }}>
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          ...
          <p style={{ fontFamily: "var(--font-oswald),sans-serif", fontSize: 11, color: "#E5B84B", letterSpacing: "0.4em", marginTop: 12 }}>
            ADMIN — TEAM CONSOLE
          </p>
        ...
        >
          ...
          />
          {error && (
            <div style={{ marginTop: 12, padding: "10px 14px", background: "#1e0d19", border: "1px solid #460a1c", color: "#ff6982", fontSize: 13 }}>
              {error}
            </div>
          )}
          <button
...
function Dashboard({ pw, onLogout }: { pw: string; onLogout: () => void }) {
  ...
  const api = useCallback(async <T,>(path: string, body: Record<string, unknown>, opts?: { silent?: boolean }): Promise<T | null> => {
    ...
    try {
      ...
      if (!opts?.silent) showToast(false, "ネットワークエラーが発生しました。");
      return null;
    } finally {
      if (isWrite) {
   ...
   *
   * Apps Script は1往復が重く、同時に何本も叩くと順番待ちで
   * 「応答が遅くタイムアウトしました」になりやすい。起動時やタブ切替で
   * 複数の一覧を読む場面をまとめることで、呼び出し回数を大幅に減らす。
   */
  const batchRef = useRef<{
    sheets: Set<string>;
    waiters: { sheet: string; resolve: (v: ListRow[] | null) => void }[];
    timer: ReturnType<typeof setTimeout> | null;
  } | null>(null);

  const fetchSheetBatched = useCallback((sheet: string): Promise<ListRow[] | null> => {
    return new Promise<ListRow[] | null>(resolve => {
      if (!batchRef.current) batchRef.current = { sheets: new Set(), waiters: [], timer: null };
      const b = batchRef.current;
      b.sheets.add(sheet);
      b.waiters.push({ sheet, resolve });
      if (b.timer) clearTimeout(b.timer);
      // 30ms の集約ウィンドウ。この間に来た要求はまとめて1回で取りに行く。
      b.timer = setTimeout(async () => {
        batchRef.current = null;
        const sheets = [...b.sheets];
        const data = await api<{ ok: true; sheets: Record<string, ListRow[]> }>(
          "/api/admin/list", { sheets }, { silent: true },
        );
        if (!data) {
          // 失敗時のエラー表示はこの1回だけ（シートごとに出すと通知が溢れるため）
          showToast(false, "最新データの取得に失敗しました。表示は前回の内容です。「再読込」でやり直せます。");
          b.waiters.forEach(w => w.resolve(null));
          return;
        }
        const map = data.sheets ?? {};
        b.waiters.forEach(w => w.resolve(map[w.sheet] ?? []));
      }, 30);
    });
  ...
  const listCached = useCallback(async (sheet: string, apply: (rows: ListRow[]) => void) => {
    ...
    const hit = readCacheWithAge<ListRow[]>(ck);
    if (hit) apply(hit.v);
    // 直近30秒以内に取得済みなら通信しない（タブを行き来しても待たされない）
    if (hit && hit.age < 30_000) { setLoadingFor(sheet, false); return; }
    setLoadingFor(sheet, !hit);
    const rows = await fetchSheetBatched(sheet);
  ...
  const loadAccounts = useCallback(async () => {
    ...
    const data = await api<{ ok: true; accounts: { id: string; name: string; status: string; createdAt: string; memberId?: string; userId?: string; rowIndex: number }[] }>("/api/admin/accounts", { op: "list" }, { silent: !!cachedAcc });
    ...
    <div className="admin-dark" style={{ minHeight: "100vh", fontFamily: IOS_FONT, background: "#000000", color: "#fff" }}>
      ...
      {(() => {
        ...
        const groups: { key: string; label: string; items: [Tab, string, number | undefined, boolean][] }[] = [
          ...
          { key: "game", label: "試合", items: [
            ...
            ["fielding", "守備記録", fielding.length, false],
            ["probables", "予告先発", probables.length, false],
          ] },
          { key: "ops", label: "運営", items: [
            ["stats", "統計", undefined, false],
          ...
          <div style={{ background: "#0F1626", borderBottom: "1px solid #38383A" }}>
            <div className="max-w-[1280px] mx-auto px-4 md:px-8" style={{ paddingTop: 12, paddingBottom: 12 }}>
              ...
              <div style={{ display: "flex", gap: 6, marginTop: 10, overflowX: "auto", WebkitOverflowScrolling: "touch", paddingBottom: 2 }}>
                {activeGroup.items.map(([key, label, count, alert]) => {
                  const on = tab === key;
                  return (
                    <button
                      key={key}
                    ...
                    >
                      ...
                      {label}
                      {count !== undefined && (
                        <span style={{ marginLeft: 6, fontSize: 11, opacity: 0.65, fontFamily: "var(--font-oswald),sans-serif" }}>{count}</span>
                      )}
                    </button>
                  );
                })}
      ...
      {saving && (
        ...
        >
          <div style={{
            background: "#131a2c",
            border: "1px solid #5b4c1f",
            padding: "16px 24px",
            display: "flex",
            alignItems: "center",
            gap: 14,
            boxShadow: "0 16px 48px #04060a",
            fontFamily: "var(--font-zen),sans-serif",
            fontWeight: 700,
            ...
            <span style={{
              width: 18, height: 18,
              border: "2px solid #3d351d",
              borderTopColor: "#E5B84B",
              borderRadius: "50%",
...
function H3({ children }: { children: React.ReactNode }) {
  return (
    <h3 style={{ fontFamily: "var(--font-zen),sans-serif", fontSize: 16, fontWeight: 900, color: "#E5B84B", marginBottom: 12, letterSpacing: "0.06em" }}>
      {children}
    </h3>
  );
}
                  ...
                  <tr key={m.id || i} style={{ borderBottom: "1px solid #38383A" }}>
                    ...
                    <Td>
                      <div style={{ display: "flex", gap: 4, justifyContent: "flex-end" }}>
                        <button onClick={() => startEdit(m)} style={{ padding: "4px 10px", background: "#28251b", color: "#E5B84B", border: "1px solid #51441e", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>編集</button>
                        <button onClick={() => remove(m)} style={{ padding: "4px 10px", background: "#280c1a", color: "#ff6982", border: "1px solid #50091c", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>削除</button>
                      </div>
                    </Td>
                  </tr>
                ))}
...
function AttendanceTab({
  ...
  const colors: Record<string, string> = {
    "出席": "#1a9f3a", "欠席": "#d10024", "遅刻": "#E5B84B", "未定": "#5b6373",
  };
                  ...
                  return (
                    ...
                    >
                      <input
                        ...
                        style={{ width: 18, height: 18, accentColor: "#E5B84B", cursor: "pointer" }}
                      />
                      <div>
          ...
          {slots.map((slot, i) => {
            ...
            return (
              <div key={i} style={{ display: "grid", gridTemplateColumns: "32px 1fr 110px", gap: 6, alignItems: "center" }}>
                <span style={{ fontFamily: "var(--font-oswald),sans-serif", fontSize: 18, color: "#E5B84B", textAlign: "center", fontWeight: 700 }}>{i + 1}</span>
                <select
                  value={slot.memberId}
      ...
      <section style={cardStyle}>
        ...
        <div style={{ background: "#070a11", padding: "16px 18px", border: "1px solid #38383A", marginBottom: 14 }}>
          ...
          <div className="grid gap-3" style={{ gridTemplateColumns: "1fr 1fr 1fr 180px" }}>
            ...
            <BSOCounter label="STRIKE" value={strikes} max={3} color="#E5B84B" onClick={addStrike} onReset={() => setStrikes(0)} />
                  ...
                  <tr key={i} style={{ borderBottom: "1px solid #38383A", background: editingRow === b._row ? "#1e1d1a" : undefined }}>
                    ...
                    <Td>
                      <div style={{ display: "flex", gap: 5 }}>
                        <button onClick={() => startEdit(b)} style={{ padding: "3px 8px", background: "transparent", color: "#E5B84B", border: "1px solid #5b4c1f", fontSize: 10, cursor: "pointer", whiteSpace: "nowrap" }}>編集</button>
                        <button onClick={() => remove(b)} style={{ padding: "3px 8px", background: "transparent", color: "#ff6982", border: "1px solid #460a1c", fontSize: 10, cursor: "pointer" }}>×</button>
                      </div>
                    </Td>
                  </tr>
                ))}
      ...
      <section style={cardStyle}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <div style={{ padding: "10px 12px", background: "#161719", borderLeft: "3px solid #E5B84B", fontSize: 12 }}>
            この試合の盗塁阻止率: <strong style={{ color: "#E5B84B", fontFamily: "var(--font-oswald),sans-serif", fontSize: 16 }}>{(csRate * 100).toFixed(1)}%</strong>
          </div>
          <button onClick={submit} disabled={saving} style={{ ...btnPrimaryStyle, marginTop: 4, opacity: saving ? 0.6 : 1, cursor: saving ? "not-allowed" : "pointer" }}>
            {saving ? "保存中…" : "記録する →"}
          </button>
                ...
                {sortedCatching.map((c, i) => {
                  ...
                  return (
                    <tr key={i} style={{ borderBottom: "1px solid #38383A" }}>
                      ...
                      <Td>
                        <button onClick={() => remove(c)} style={{ padding: "3px 8px", background: "transparent", color: "#ff6982", border: "1px solid #460a1c", fontSize: 10, cursor: "pointer" }}>×</button>
                      </Td>
                    </tr>
                  );
                })}
                ...
                {sorted.map((f, i) => {
                  ...
                  return (
                    <tr key={i} style={{ borderBottom: "1px solid #38383A" }}>
                      ...
                      <Td><span style={{ color: "#E5B84B", fontFamily: "var(--font-oswald),sans-serif" }}>{r.toFixed(3).replace(/^0/, "")}</span></Td>
                      <Td>
                        <button onClick={() => remove(f)} style={{ padding: "3px 8px", background: "transparent", color: "#ff6982", border: "1px solid #460a1c", fontSize: 10, cursor: "pointer" }}>×</button>
                      </Td>
                    </tr>
                  );
                })}
      ...
      <section style={cardStyle}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, cursor: "pointer", padding: "8px 0" }}>
            <input type="checkbox" checked={notify} onChange={e => setNotify(e.target.checked)} style={{ width: 18, height: 18, accentColor: "#E5B84B" }} />
            保存と同時にメンバーへ通知する 🔔
          </label>
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 26 }}>
        <div style={{ fontFamily: "var(--font-zen),sans-serif", fontWeight: 800, fontSize: 13, color: "#E5B84B", marginBottom: 10, letterSpacing: "0.06em" }}>
              ...
              <section key={a.id || a._row} style={{ ...cardStyle, border: "1px solid #5b4c1f", display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  ...
                  <button onClick={() => act(a, "reject")} disabled={saving}
                    style={{ padding: "11px 16px", background: "transparent", color: "#ff6982", border: "1px solid #6e071e", fontFamily: "var(--font-zen),sans-serif", fontWeight: 700, fontSize: 13, cursor: saving ? "not-allowed" : "pointer", opacity: saving ? 0.6 : 1 }}>
                    却下
                  </button>
                </div>
              </section>
      ...
      {/* 危険な操作 */}
      <div style={{ ...cardStyle, marginTop: 26, border: "1px solid #5a081d" }}>
    ...
    <section style={{ ...cardStyle, border: "1px solid #51441e" }}>
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
        ...
        <button
          ...
          style={{ flex: 1, padding: "12px", background: "transparent", color: "#E5B84B", border: "1px solid #6f5b21", fontFamily: "var(--font-zen),sans-serif", fontWeight: 700, fontSize: 13, cursor: saving ? "not-allowed" : "pointer", opacity: saving ? 0.6 : 1 }}
        >
          編集保存
        </button>
        <button
          onClick={() => onReject(p)} disabled={saving}
          style={{ flex: 1, padding: "12px", background: "transparent", color: "#ff6982", border: "1px solid #6e071e", fontFamily: "var(--font-zen),sans-serif", fontWeight: 700, fontSize: 13, cursor: saving ? "not-allowed" : "pointer", opacity: saving ? 0.6 : 1 }}
        >
          却下
        </button>
      </div>
    ...
    <div className="grid gap-5 grid-cols-1 lg:grid-cols-[480px_1fr]">
      <section style={{ ...cardStyle, border: isOn ? "1px solid #d10024" : undefined }}>
        ...
        <div style={{
          ...
          background: isOn ? "#1e0d19" : "#0c1d1b",
          border: `1px solid ${isOn ? "#5a081d" : "#104826"}`,
        }}>
...
function MigrationCard({
  ...
  return (
    <section style={{ ...cardStyle, border: "1px solid #265540" }}>
      ...
      {done && (
        <div style={{ marginBottom: 14, padding: "12px 14px", background: done.failedCount === 0 ? "#0c1f1c" : "#27221e", border: `1px solid ${done.failedCount === 0 ? "#104826" : "#6c522c"}`, borderRadius: 10, fontSize: 12.5, lineHeight: 1.8 }}>
          {done.failedCount === 0
                ...
                {[...stats]
                  ...
                  .map(s => (
                    <tr key={s.m.id} style={{ borderBottom: "1px solid #38383A" }}>
                      ...
                      <Td><span style={{ color: "#E5B84B" }}>{s.lateCount}</span></Td>
                      <Td><span style={{ color: "#ff6982" }}>{s.absentCount}</span></Td>
                      <Td>{s.recordedDays}</Td>
                      <Td>
                        <span style={{ fontFamily: "var(--font-oswald),sans-serif", fontSize: 16, fontWeight: 700, color: "#E5B84B" }}>
